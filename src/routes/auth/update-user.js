...
router.patch('/:userId', verifyToken, async (req, res) => {
	...
	const [data, error] = await promiseHandler(Signup.findByIdAndUpdate(userId, payload, { returnDocument: 'after' }));
	...
	if (data) {
		return res.status(200).json(generateResponse(200, 'Update Success'));
	}
